...
//
static
sm_ric_if_ans_t on_subscription_slice_sm_ag(sm_agent_t* sm_agent, const sm_subs_data_t* data)
{
  ...
  sm_ric_if_ans_t ret = {.type = SLICE_RIC_IF_SUBS_ANS_V0};

  ret.slice.et = slice_dec_event_trigger(&sm->enc, data->len_et, data->event_trigger);
  ret.slice.ad = NULL;
