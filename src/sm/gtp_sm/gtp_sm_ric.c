...
static
sm_ag_if_ans_t ric_on_e2_setup_gtp_sm_ric(sm_ric_t const* sm_ric, sm_e2_setup_t const* setup)
{
  assert(sm_ric != NULL); 
  assert(setup == NULL); 
  sm_gtp_ric_t* sm = (sm_gtp_ric_t*)sm_ric;  

  sm_ag_if_ans_t ret = {.type = GTP_E2_SETUP_ANS_V0};

  ret.gtp_e2_setup = gtp_dec_func_def(&sm->enc, setup->len_rfd, setup->ran_fun_def);
